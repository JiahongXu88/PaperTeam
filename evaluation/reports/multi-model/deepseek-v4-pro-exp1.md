...
- model: `gw-anthropic/deepseek-v4-pro`（provider `gw-anthropic`，来源 cli）
- dataset: claude-compatible（M6.9.2.1 noise-token 兼容变体）
- runtime: pi（PiRuntimeAdapter，无新增调用链）
- run: 2026-09-18T10:19:43.876Z → 2026-09-18T10:20:50.473Z（67s）
- errors: 0
...
- judge 与生成同用 gw-anthropic/deepseek-v4-pro（同模型自评偏差；正式实验应引入异模型 judge）

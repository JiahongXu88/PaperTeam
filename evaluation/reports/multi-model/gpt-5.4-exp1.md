...
- model: `gw-openai/gpt-5.4`（provider `gw-openai`，来源 cli）
- dataset: claude-compatible（M6.9.2.1 noise-token 兼容变体）
- runtime: pi（PiRuntimeAdapter，无新增调用链）
- run: 2026-09-18T10:17:46.800Z → 2026-09-18T10:18:11.529Z（25s）
- errors: 0
...
- judge 与生成同用 gw-openai/gpt-5.4（同模型自评偏差；正式实验应引入异模型 judge）

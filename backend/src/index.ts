  ...
  const runtime: AgentRuntime & import("./settings/ModelSettingsService.js").ModelSettingsRuntime =
    useScriptedRuntime
      ...
      : new PiRuntimeAdapter({
          ...
          // M5.2 长程治理：rotation fallback / 会话 TTL / 容量 / 输出预留
          maxRunsPerSession: config.pi.maxRunsPerSession,
          sessionIdleTtlMs: config.pi.sessionIdleTtlMs,
          maxSessions: config.pi.maxSessions,
          ...(config.pi.outputReserveTokens !== undefined
          ...
          roleSkills: (role, scope, language) => skillRegistry.skillAssignmentsFor(role, scope, language),

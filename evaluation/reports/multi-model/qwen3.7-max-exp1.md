...
- model: `gw-anthropic/qwen3.7-max`（provider `gw-anthropic`，来源 cli）
- dataset: claude-compatible（M6.9.2.1 noise-token 兼容变体）
- runtime: pi（PiRuntimeAdapter，无新增调用链）
- run: 2026-09-18T10:20:54.482Z → 2026-09-18T10:21:39.991Z（46s）
- errors: 0
...
- judge 与生成同用 gw-anthropic/qwen3.7-max（同模型自评偏差；正式实验应引入异模型 judge）

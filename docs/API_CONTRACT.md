...
| `POST /api/projects/:id/research/web-search` | M6.3 Web 检索（SearXNG，optional）：`{query, limit?, saveAsCandidates?: number[]}` → 同上结构（results 为 WebSearchResult：url canonical 化 / title / snippet / engines / score）。未配置 SearXNG → 503 SEARCH_PROVIDER_NOT_CONFIGURED（其余能力不受影响）；JSON API 未启用（403）→ 502 附 misconfigured 说明 | M6.3 |

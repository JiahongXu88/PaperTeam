...
2. **Existing-Paper Improvement**：导入已有论文（最终 PDF 正式输入；LaTeX 项目导入为改造阶段形态），第一阶段先建立 Review 基线，再系统性理解、审计与改造。
3. **Existing-Paper Review（快速 Review，2026-09）**：导入论文 PDF 即建立项目，只读分析——引用真实性核验、论断-引用一致性、分章节审阅与汇总审阅报告；不修改论文正文。
...
## 5.1 项目入口（2026-09：二选一 + File First）

新建项目时，用户首先回答「你想做什么？」：

```text
○ 从研究想法开始（Idea-to-Paper）
● 导入已有论文（Existing Paper）
     ├─ 快速 Review（默认推荐）：PDF 解析 → 引用真实性核验 → 论断-引用一致性
     │   → 分章节审阅 → 汇总审阅报告；只读，不修改论文
     └─ 系统性改进：先 Review 建立基线，再依据审阅发现进入后续修改与优化
```

**导入已有论文是 File First 的**：用户只需选择 PDF 与目标即可创建项目——
论文标题自动取自 PDF（缺失/不可用时用文件名兜底），不要求用户手填标题，
也不先创建空项目再进工作区上传；研究领域 / 目标期刊等非必要字段折叠进
「高级选项」。导入后标题可随时编辑（PDF metadata 可能识别错误）。

项目生命周期（2026-09）：项目可**归档**（从论文项目列表与最近项目消失，
不删除任何数据；有进行中任务时拒绝归档）、**恢复**；已归档项目可在
「设置 → 项目管理」中**永久删除**（输入完整项目标题确认；删除整个工作区，
不可恢复）。

三类项目共用同一套 Workspace 结构、文献库、Evidence Store、审稿与修改闭环；
差异只在前置阶段（调研/可行性 vs PDF 导入/解析/审阅 vs LaTeX 导入/理解/审计）
与最终产物形态。
...
- **M2.1 OpenClaw 2.0 Runtime Upgrade**：官方 Gateway SDK（2026.8.1 起，M3.5 补丁至
  2026.8.2、M3.6 升至 2026.9.1，protocol v4）、Project ↔ Session 隔离与 runtimeSessionKey 持久化
